...
El proyecto es un maquetado simple de react.
...
* [Para correr el proyecto en un servidor local](webpack-dev-server), primero debemos instalar las dependencias.
  Para eso nos vamos a la consola y parados en la raiz del proyecto corremos `npm install`.
  Esperamos la instalacion de los modulos de node y ejecutamos `npm start`.
  Esto abrira el navegador y la aplicación debera encontrarse corriendo el localhost:3000. En caso de que algo mas este corriendo en el puerto 3000, se preguntara en la consola si desea correr la app en otro puerto, corres `y`.

* [Para modificar] la configuración de Webpack(pocas veces necesario), podemos correr el comando `npm run eject`. Esto quitara la dependecia de `react-scripts` y agregara la carpeta de configuración y los archivos correspondientes.

* [Para compilar el proyecto] debemos correr `npm run build`. Esto nos agregara una carpeta llamada "build", la cual contiene todos los archivos necesarios(HTML, CSS, Javascript) para subir al servidor de producción.
